...
def _detect_page_type(context: str, style_results: list) -> str:
    ...
    if style_results:
        style_category = style_results[0].get("Style Category", "").lower()
        best_for = style_results[0].get("Best For", "").lower()

        # 首先检查style_category
        if "dashboard" in style_category or "data" in style_category or "analytics" in style_category:
            return "Dashboard / Data View"
        elif "landing" in style_category or "marketing" in style_category or "promo" in style_category:
            return "Landing / Marketing"
        # 如果style_category没有匹配，则回退到best_for
        elif "dashboard" in best_for or "data" in best_for:
            return "Dashboard / Data View"
