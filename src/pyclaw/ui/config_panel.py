...
def build_config_panel(*, gateway_client: Any = None) -> ft.Column:
    theme = get_theme()
    ...
    async def _load_config() -> None:
        if not gateway_client or not gateway_client.connected:
            return
        try:
            result = await gateway_client.call("config.get")
            config_ref.clear()
    ...
    async def _save_config() -> None:
        ...
        try:
            ...
            obj = json.loads(text)
            await gateway_client.call("config.save", {"config": obj})
            config_ref.clear()
    ...
    async def _apply_config() -> None:
        if not gateway_client or not gateway_client.connected:
            return
        try:
            await gateway_client.call("config.apply")
        except Exception:
            pass
    ...
    def _on_form_change(section: str, key: str, value: Any) -> None:
    ...
    async def _save_form_and_sync() -> None:
        if not gateway_client or not gateway_client.connected:
            return
        try:
            await gateway_client.call("config.save", {"config": config_ref})
            raw_text.value = json.dumps(config_ref, indent=2, ensure_ascii=False)
            _safe_update(raw_text)
        except Exception:
            pass
    ...
    async def _load_schema() -> bool:
        if not gateway_client or not gateway_client.connected:
            return False
        try:
            result = await gateway_client.call("config.schema")
            schema_ref.clear()
