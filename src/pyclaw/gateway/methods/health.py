...
def create_health_handlers() -> dict[str, MethodHandler]:
    async def handle_health(params: dict[str, Any] | None, conn: GatewayConnection) -> None:
        await conn.send_ok(
            "health",
            {
                "status": "ok",
    ...
    async def handle_status(params: dict[str, Any] | None, conn: GatewayConnection) -> None:
        await conn.send_ok(
            "status",
            {
                "status": "ok",
