...
def create_config_handlers(*, config_path: str | None = None) -> dict[str, MethodHandler]:
    ...
    async def handle_config_get(params: dict[str, Any] | None, conn: GatewayConnection) -> None:
    ...
    async def handle_config_set(params: dict[str, Any] | None, conn: GatewayConnection) -> None:
        ...
        if not params or "config" not in params:
            await conn.send_error("config.set", "invalid_params", "Missing 'config' in params.")
            return
    ...
    async def handle_config_patch(params: dict[str, Any] | None, conn: GatewayConnection) -> None:
        from pyclaw.config.io import patch_config

        if not params or "patch" not in params:
            await conn.send_error("config.patch", "invalid_params", "Missing 'patch' in params.")
            return
