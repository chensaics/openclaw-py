...
def _apply_fixes(findings: list[AuditFinding], config_path: Path) -> None:
    ...
    for f in findings:
        if f.category == "filesystem" and "world/group readable" in f.message and config_path.exists():
            try:
        ...
        if f.category == "gateway" and "0.0.0.0" in f.message and config_path.exists():
            try:
